...
#include <math.h>
#include<time.h>
#include "subrounte_newton_qsv.c"
#include <omp.h>
char *sdoc[] = {
...
int main()
{
	...
	sprintf(filename,"../para/epsilon1.bin");
	if((fp=fopen(filename,"rb"))==NULL){
		printf("cannnot open %s\n",filename);
		exit(0);
	}
	...
	sprintf(filename,"../para/delta1.bin");
	if((fp=fopen(filename,"rb"))==NULL){
		printf("cannnot open %s\n",filename);
		exit(0);
	}
	for(ix=2;ix<nx-2;ix++){
		for(iz=2;iz<nz-2;iz++){
			fread(&a,sizeof(float),1,fp);//a=0.1;
			delta[ix][iz]=(double)a;
		}
	}
	fclose(fp); 

	sprintf(filename,"../para/theta1.bin");
	if((fp=fopen(filename,"rb"))==NULL){
		printf("cannnot open %s\n",filename);
		exit(0);
	}
	...
	fp=fopen("../data/qsv_newton.bin","wb");
	for(i=2;i<nx-2;i++){
	    for(j=2;j<nz-2;j++){  
		fa=(float)T[i][j];
